...
public class SueloCanvas extends Canvas {
  ...
  int xx, yy; // coordenadas de todo el canvas con respecto al componente que lo contendrá
  int paso;
  int paredH[][];
  int paredV[][];
  int numAge;
...
//    setBackground(colorSuelo);
    ...
    /*
     *  Definición de los colores de los agentes
     */
    ...
    valor = 1;
/*    agentes = new int[2][3];  // []--> núm. de agentes, [2]---> orientacion y coordenadas (x,y)
    numAge = 2;*/
  ...
  /*
   *  Los siguientes métodos se refieren a colocar al Agente (polígono - triángulo)
   *  sobre el suelo con una orientación
   */

  public void setAgentes(int num){
    agentes = new int[num][3];  // []--> núm. de agentes, [2]---> orientacion y coordenadas (x,y)
    numAge = num;
    // TEMPORAL
    for(int i=0; i< num; i++){
       setAgenteXY(i,NORTE,4,2+i); // CORREGIR -- BUSCAR UNA CELDA VACÍA
    }
    repaint();
  }
  ...
  public int isAgente(int x, int y){
    //checa si no existe ya en algún otra celda
    int i;
    x = convertir(x);
    y = convertir(y);
  ...
  /*
   *  Sobrecarga del método update para eliminar el parpadeo
   */
  ...
  /*
   *  Sobrecarga del método paint propio del Canvas
   */
  ...
  public void paint(Graphics g){
    ...
    if(tempPos[0]==-1 && tempPos[1]==-1){
      g2.setColor(colorSuelo); // especifica el color del espacio gráfico
      g2.fillRect(this.getX(), this.getY(), this.getWidth(), this.getHeight()); // Hace un rectánculo, y lo colorea de azul
    }
    ...
    g2.setColor(colorLineas); // especifica el color del espacio gráfico

    for(i=0; i<=this.getWidth(); i= i+paso){ // dibuja las líneas verticales
      g2.drawLine(i, 0, i, this.getHeight());
    }
    for(i=0; i<=this.getHeight(); i= i+paso){  // dibuja las líneas horizontales
    ...
    // Coloca las imágenes (es importante que sea después de las sombras,
    ...
    // AGENTES  (es importante que sea la última en dibujarse en el panel)
    for(x=0; x<numAge; x++){
    ...
    if(x1!= x2 & y1==y2){ // paredes horizontales
      ...
      for(; x1<x2; x1++){
        paredH[x1][y1] = 1;
         ...
         if (x1 == x2 & y1 != y2) { // se valida que haga referencia a una línea, y no a un punto
           if(y1>y2){ // identifica qué punto es el menor
             temp = y1;
             y1 = y2;
             y2 = temp;
           }
  ...
  private boolean isOcupada(int x, int y){
    ...
    // verifica en la celdas si existe algún objeto
    if(celda[x][y]!=0){
      return true;
    }
    return false;
  ...
  // QUITAR OBJETOS : OBSTÁCULOS, LUZ, TEMPERATURA
  ...
  public void quitarPared(int x, int y){
    ...
    if(ry<=holgura || ry>=paso-holgura){ // condición que hace referencia
    ...
    if(!bandx && bandy){ // Línea Horizontal
      if(paredH[x][y]==1) quitarParedH(x,y); // si existe pared Horizontal
      else
        JOptionPane.showMessageDialog(null,
                                      "No existe pared en esta área",
                                      "Alerta",
                                      JOptionPane.ERROR_MESSAGE);
    }

    if(bandx && !bandy){ // Línea / Pared Vertical
    ...
    if(bandx && bandy){  // Existen dos paredes (vertical y horizontal) encontradas en un punto
      if(paredV[x][y]==1 && paredH[x][y]==1){
        JOptionPane.showMessageDialog(null,
                                      "Especifique con mayor precisión la pared a borrar",
                                      "Alerta",
                                      JOptionPane.ERROR_MESSAGE);
      }else{
    ...
    if(!bandx && !bandy){// No se señaló un área valida donde se encuentre dibujada la pared
      if(celda[x][y]==PARED0 || celda[x][y]==PARED1){
...
/*    for(i=0; i<this.getWidth()/paso; i++){ // avanza las y's
  ...
  /*
   * Los siguientes métodos abren un mundo.
   * Y además, guardan el mundo diseñado sobre el suelo.
   */
  ...
  public void abrirMundo(String ruta){
   ...
   temp = new int[4];
   limpia();
   // Carga las líneas del archivo *.mdo
   c = in.getCodigo();
 ...
 /*
  *  Los siguientes métodos se refieren a las primitivas del lenguaje Age2000
  *  (avanzar, retroceder, gira a la derecha, gira a la izquierda).
  *  Además, el último método, se refiere a dibujar un polígono (triángulo)
  *  en la nueva orientación o celda.
  */
...
// Dibuja Paredes Diagonales
  public Polygon diagonal(int a, int b, int pared){
    ...
    // '\'
    if(pared == PARED1){
      objeto.addPoint( (a * paso) + 2, b * paso); // Forman un triángulo en la punta (cada 3 puntos)
...
// Función temporal, está por definirse su estado
  ...
  public int undo(){
    ...
    if(i>0){
      ...
      switch (nodo.getTipo()) {
        case 1:// ORIENTACION : Recuperar la orientación original
  ...
  public int redo(){
    ...
    if(i>0){
      ...
      switch (nodo.getTipo()) {
        case 1:// ORIENTACION: Recuperar la orientación ejecutada
  ...
  /*
   * Las siguientes funciones permiten el conocer en qué posición se encuentra
   * el agente, y reportar obstáculos, paredes, luz, y temperatura en su camino.
   */
...
// TACTIL
  ...
  public boolean getTactilAgente(int opcion, int id){ // Función tomando en cuenta al agente hacia el norte
    boolean band = false;
    switch(opcion){
      case IZQUIERDA:// Este caso se refiere a algún obstáculo, ó pared del lado IZQUIERDO
                     // sin tomar en cuenta la orientación del agente
                     ...
                     break;
      case DERECHA:  // Este caso se refiere a algún obstáculo, ó pared del lado DERECHO
                     // sin tomar en cuenta la orientación del agente
...
// TEMPERATURA
  ...
  public int getTemperaturaAgente(int opcion, int id){ // Función tomando en cuenta al agente hacia el norte
    switch(opcion){
      case IZQUIERDA:// Este caso se refiere a algún obstáculo, ó pared del lado IZQUIERDO
                     if(agentes[id][1]>0){ // si hay celda del lado izquierdo
