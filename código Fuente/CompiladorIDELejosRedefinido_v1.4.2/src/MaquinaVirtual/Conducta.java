...
public class Conducta extends Thread{
   ...
   private void switchInstrucciones(){
     ...
     switch(entrada){
       case '*':int id = in.getCodigo();   // Solicita
       			System.out.println("Llega a solicitar");
       			System.out.println("solicita: "+nombre+" con Id: "+id);
       			agente.tablaConductas.actualiza_uso_tipo(nombre, true, id);
       ...
       case '=':if(in.getCodigo()=='$'){ // Detecta una asignación
         ...
         case 22: caracter = in.getCodigo();  // Evalúa la condición REPITE
                  ...
                  if(in.getCodigo()=='{'){// llaves de inicio de instrucciones (INICIO)
                	  System.out.println("EMPIEXA A VERIFICAR ACIONNES");
  ...
  if(caracter == 64){ // '@'  Si se declaran las variables tipo Entero y Decimal
    ...
    do{
        if (caracter == '$') { // Indicador de variables
...
do{
  switch(caracter){
    ...
    case '&': double yy= getSensor();  // Obtiene el valor del sensor en cuestión ------ !!!!! CORREGIR!!!!!!!!!!!!!
              pilaNum.push(new Double(yy));
              break;
...
private double getSensor(){
  double temp = 0.0;
  int id = in.getCodigo();
  temp = agente.tablaSensores.buscarIdValor2(id);
  return temp;
